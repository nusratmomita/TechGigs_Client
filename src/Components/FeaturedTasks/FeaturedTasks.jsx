...
const FeaturedTasks = ({data}) => {
    ...
    return (
        <div  className="lg:-mt-40 p-10">
            <h2 className="text-7xl text-center font-extrabold text-blue-400 mb-10">| Featured Tasks |</h2>
            <div  className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {data.map(task => (
                    <div key={task._id} className="h-[350px] w-[360px] border p-6 rounded-xl shadow-lg bg-white">
                        ...
                        <Link to={`/detailedTask/${task._id}`}>
                            <button className='btn btn-primary mt-20'>See More</button>
                        </Link>
                    </div>
                ))}
            </div>
